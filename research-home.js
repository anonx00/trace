...
export function mountResearchHome(root,{topicCount=0,openSearch}={}) {
  ...
  function draw() {
    ...
    for(const {d,x,y} of domainPoints) {
      ...
      nodes+=`<g class="map-node atlas-domain-node ${muted?'scope-muted':''}" data-domain-id="${d.id}" data-route="#/domain/${d.id}" role="link" tabindex="0" aria-label="${esc(d.name)}" transform="translate(${x} ${y})" style="--node:${d.color}"><title>${esc(d.name)} — open domain</title>${detail?`<rect class="atlas-domain-header" x="-138" y="-24" width="276" height="44" rx="8"/><path class="atlas-node-icon" d="${glyphs[d.id]}" transform="translate(-113 0) scale(.65)"/><text class="atlas-node-title" x="-87" y="5">${esc(d.short)}</text><text class="atlas-node-count" x="121" y="4">${String(d.services.length).padStart(2,'0')}</text>`:`<circle class="atlas-node-aura" r="65"/><circle class="atlas-node-orbit" r="50"/><circle class="atlas-node-disc" r="33"/>${satellites}<path class="atlas-node-icon" d="${glyphs[d.id]}"/><text class="atlas-node-title" y="90">${esc(d.short)}</text><text class="atlas-node-count" y="109">${String(d.services.length).padStart(2,'0')} SERVICES</text>`}</g>`;
      if(detail) d.services.forEach(id=>{const p=positions.get(id),s=serviceById(id);links+=`<path class="atlas-membership ${muted?'scope-muted':''}" d="M${x-132},${y+24} V${p.y} H${p.x-120}"/>`;nodes+=`<g class="map-node atlas-service-node ${muted?'scope-muted':''}" data-domain-id="${d.id}" data-service-id="${id}" data-route="#/service/${id}" role="link" tabindex="0" aria-label="${esc(s.name)}" transform="translate(${p.x} ${p.y})" style="--node:${d.color}"><title>${esc(s.name)}</title><rect class="atlas-service-card" x="-120" y="-17" width="248" height="34" rx="7"/><circle class="atlas-service-core" cx="-102" r="3.5"/><text x="-88" y="5">${esc(s.short)}</text><text class="atlas-service-arrow" x="108" y="5">↗</text></g>`;});
    }
  ...
  const loadStory=()=>{mountStory(storyHost,storyScenario);if(storyMode){const service=serviceById(storyScenario.stages[0].service);showPreview(domainFor(service.id),service);}};
  loadStory();
