...
export function knowledgeGraph({type,d,s,topic,topicIndex,doc,tab,page=0,references=[]}) {
  ...
  visible.forEach((v,i)=>{
    ...
    const summary=profile?(tab==='evidence'?profile.evidence[0]:tab==='defense'?profile.defenses[0]:v.description||profile.boundary):v.description;
    const lens=tab==='evidence'&&profile?'Evidence':tab==='defense'&&profile?'Defense':v.relation;
