...
export function knowledgeGraph({type,d,s,topic,topicIndex,doc,tab,page=0,references=[]}) {
  ...
  visible.forEach((v,i)=>{
    ...
    const lens=service&&tab==='evidence'?'Evidence':service&&tab==='detections'?'Detections':service&&tab==='defense'?'Defense':v.kind==='documented'?'AWS capability':v.kind==='attack'?'Scenario sequence':v.kind==='references'?'Guide reference':service?domain.short:'AWS guide';
    ...
    nodes+=`<g class="map-node mind-graph-node" tabindex="0" role="link" aria-label="${esc(title)}" data-route="${esc(v.url)}" data-edge-index="${i}" data-incoming="${!!v.incoming}" data-kind="${v.kind}" transform="translate(${x},${y})" style="--color:${color}"><title>${esc(title)} — ${esc(v.relation)}</title><rect class="mind-node-back" width="284" height="164" rx="12"/><g class="mind-neuron" transform="translate(34 43)"><rect class="mind-neuron-body" x="-20" y="-20" width="40" height="40" rx="9"/><path class="mind-neuron-icon" d="${glyphs[domain.id]}" transform="scale(.65)"/></g><foreignObject x="66" y="22" width="198" height="142"><div xmlns="http://www.w3.org/1999/xhtml" class="mind-node-copy"><span>${esc(lens)}</span><h3>${esc(title)}</h3><p>${esc(v.kind==='membership'&&service?service.label:v.relation)}</p></div></foreignObject></g>`;
