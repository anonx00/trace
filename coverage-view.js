...
export function mountCoverage(root,{documents=[],docsAvailable=true}={}) {
  ...
  function render(){
    ...
    const count=(row,value,label)=>{const href=label==='research paths'?`#/paths?service=${row.service.id}`:`#/service/${row.service.id}${label==='imported topics'?'?tab=docs':label==='reviewed rule mappings'?'?tab=detections':''}`;return `<td><a href="${href}" class="coverage-value ${value===null?'unknown':value?'mapped':'gap'}" aria-label="${esc(row.service.name)}: ${value===null?'unknown':value} ${label}"><i aria-hidden="true"></i>${value===null?'Unknown':value||'—'}</a></td>`;};
